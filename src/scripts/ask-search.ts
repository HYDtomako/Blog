...
export type SearchData = { answers: AnswerResult[]; articles: ArticleResult[]; notes?: ArticleResult[] };
...
export type AskSearchConfig = Partial<AskSearchLimits> & {
	...
	/** Search index for the current locale (deploy base already applied). */
	indexHref?: string;
	/** Locale path prefix for ask links (`''` for the default locale). */
	askPrefix?: string;
};
...
const defaultLabels: AskSearchLabels = {
	...
	noResults: 'Nothing matched. Try different wording.',
};
...
export function createAskSearch(
  ...
  const answerNode = (answer: AnswerResult) => {
    ...
    link.href = askPageUrl(answer.q, undefined, config.askPrefix ?? '');
  ...
  const renderResults = (query: string) => {
    ...
    const docs = [...searchData.articles, ...(searchData.notes ?? [])];
    const articles = normalized
      ...
      : docs.slice(0, limits.articlesEmpty);
    ...
    if (answers.length === 0 && articles.length === 0) {
      const empty = element("div", "overlay-state");
    ...
    if (config.showAskFallback && query.trim()) {
      ...
      askLink.href = askPageUrl(query.trim(), undefined, config.askPrefix ?? '');
  ...
  const loadSearchData = async (force = false) => {
    ...
    onResultsRendered?.();
    try {
      const response = await fetch(config.indexHref ?? withBase("/api/search-index.json"), {
        cache: "no-store",
      });
