...
/**
 * Link to the ask page, optionally prefilled (includes deploy base and a locale
 * prefix such as `/en`). Kept free of site config so client scripts can import it.
 */
export function askPageUrl(query = '', base?: string, localePrefix = '') {
	const q = query.trim();
	const path = q ? sitePath(`${localePrefix}/ask?q=${encodeURIComponent(q)}`) : sitePath(`${localePrefix}/ask`);
