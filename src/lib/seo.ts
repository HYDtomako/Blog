...
import { getSiteCopy, SITE_BRAND, type SiteCopy } from './site-copy';
...
function pageTitle(entry: CollectionEntry<'docs'>, logicalPath: string, copy: SiteCopy) {
	if (logicalPath === localePath(copy.locale, '/')) return copy.brand.homeTitle;
...
export function normalizeSeoHead({
	...
	const title = pageTitle(entry, logical, copy);
