...
const locales: readonly string[] = siteConfig.locales.list;
const defaultLocale: string = siteConfig.locales.default;
...
/** NLWeb schemas describe the optional remote contract, so they only ship when a remote is configured. */
function buildSchemas(includeAsk: boolean) {
	return {
		Profile: {
			type: 'object',
...
const staticReads: Array<{
...
}> = [
	{
		...
		label: { 'zh-CN': '公开个人资料', en: 'Public profile' },
	},
	{
		path: '/api/articles.json',
		operationId: 'getArticles',
		schema: { type: 'object' },
		label: { 'zh-CN': '公开文章索引', en: 'Public article index' },
	},
	{
		path: '/api/topics.json',
		operationId: 'getTopics',
		schema: { type: 'object' },
		label: { 'zh-CN': '公开主题索引', en: 'Public topic index' },
	},
	{
		path: '/api/search-index.json',
		operationId: 'getSearchIndex',
		schema: { type: 'object' },
		label: { 'zh-CN': '公开静态搜索索引', en: 'Public static search index' },
	},
];

function localePrefixFor(locale: string) {
	return locale === defaultLocale ? '' : `/${locale}`;
}

function localeNote(locale: string) {
	const prefix = localePrefixFor(locale);
	return prefix
		? `Served from the ${locale} locale tree under ${prefix}/.`
		: `Served from the default locale tree (${locale}) at the site root.`;
}

function localeOperationId(operationId: string, locale: string) {
	if (locale === defaultLocale) return operationId;
	const suffix = locale
		.split(/[^A-Za-z0-9]+/)
		.filter(Boolean)
...
function buildStaticReadPaths(paths: OpenApiDocument['paths']) {
	for (const locale of locales) {
		const prefix = localePrefixFor(locale);
		for (const read of staticReads) {
			paths[`${prefix}${read.path}`] = {
				get: {
					operationId: localeOperationId(read.operationId, locale),
					description: localeNote(locale),
					responses: {
						200: response(read.label[locale] ?? read.label[defaultLocale], read.schema),
					},
				},
			};
		}
...
export function buildOpenApiDocument(caps: PublicCapabilities): OpenApiDocument {
	const schemas = buildSchemas(Boolean(caps.ask || caps.mcp));
	const paths: OpenApiDocument['paths'] = {};
	buildStaticReadPaths(paths);
	...
	return {
		...
		info: {
			...
			description:
				caps.mode === 'static'
					? `Build-time read-only JSON APIs for ${caps.identity.brand}. No remote Ask or MCP endpoint is configured, so this document declares static read paths only and the /ask/ UI searches the static corpus. ${caps.unsupportedNotes} Supported locales: ${locales.join(', ')} (default: ${defaultLocale}). Every static read path is documented once per locale tree.`
					: `Build-time read-only JSON APIs plus ${caps.capability}. Static JSON is for indexing and mirrors; Ask/MCP are optional remote endpoints when configured. ${caps.supportedNotes} ${caps.unsupportedNotes} Supported locales: ${locales.join(', ')} (default: ${defaultLocale}). Every static read path is documented once per locale tree.`,
		},
